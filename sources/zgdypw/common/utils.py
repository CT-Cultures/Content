...
class DB(object):
    ...
    @staticmethod
    def convert_cjk_characters(s:str) ->str:
      """
      This functions converts cjk encoded characters to their
      unicode equivalent.
      """

      df_cjk2unicode = pd.read_json('common/cjk_radical_to_unicode.json')
      ChineseChar_cjk2unicode = {}
      for k, v in zip(df_cjk2unicode['char_cjk_radical'].to_numpy(), 
                      df_cjk2unicode['char_unicode'].to_numpy()):
          ChineseChar_cjk2unicode[k] = v

      for k, v in ChineseChar_cjk2unicode.items():
        s = s.replace(k, v)
