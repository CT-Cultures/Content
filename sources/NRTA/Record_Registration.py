...
class Registration(object):
    ...
    def __init__(self):
        ...
        # read registration records to  ignnore, prior to 2012
        ...
        # read existing links of registrations
        ...
        for link, name in zip(links_of_publications['公示批次链接'].tolist(), 
            ...
            with urlopen(req, context=ctx) as x:
                ...
                for tr in table.findAll("tr"):
                    ...
                    for each in trs:
                        try:
                            link_reg = each.find('a')['href']
                            ls_link_reg.append(self.url_base + '/' + link_reg)
                        except:
                            pass                                
                df['备案链接'] = ls_link_reg                
                df['公示名称'] = name
    ...
    def update_records(self, 
        ...
        links_of_publications_latest = links_of_publications_latest[
            ~links_of_publications_latest['公示名称'].isin(
                self.pub_to_ignore_existing['公示名称'])
            ]
        ...
        if how == 'comprehensive':
            ...
            links_of_registrations_new = links_of_registrations_latest[
                ~links_of_registrations_latest['备案链接'].isin(
                    self.contents_of_registrations_existing['备案链接'])
            ]
        ...
        content_of_registrations_latest = \
            contents_of_registrations_latest.sort_values(
                '公示年月').reset_index(drop=True)
        
        content_of_registrations_latest = \
            content_of_registrations_latest.sort_values(
                '公示年月', ascending=False)
